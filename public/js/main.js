...
function handleHit(shape, result, x, y) {
    ...
    if (collectAllActive) {
        ...
        shapes = [spawnRandomShape(canvas.width, canvas.height, score)];
        collectAllActive = false;
...
function pushTap(geometry, result) {
    ...
    // Keep last 20 taps
    if (tapHistory.length > 20)
        tapHistory.shift();
}
function triggerPattern(pattern, shape) {
    const result = activatePatternEffect(pattern.effect, shapes, canvas.width, canvas.height, score, particles);
...
function clearScreen() {
    const bgColor = getComputedStyle(document.documentElement).getPropertyValue('--c-bg').trim() || '#0a0a1a';
...
function renderMenu() {
    ...
    ctx.font = `bold ${Math.min(canvas.width, canvas.height) * 0.06}px Inter, system-ui, sans-serif`;
    ctx.textAlign = 'center';
    ctx.fillText('☀️ Glow', canvas.width / 2, canvas.height / 2 - 20);
    ctx.fillStyle = 'rgba(255, 255, 255, 0.5)';
    ctx.font = `${Math.min(canvas.width, canvas.height) * 0.025}px Inter, system-ui, sans-serif`;
...
function renderGameOver() {
    ...
    ctx.font = `bold ${Math.min(canvas.width, canvas.height) * 0.05}px Inter, system-ui, sans-serif`;
    ctx.textAlign = 'center';
    ctx.fillText(reason, canvas.width / 2, canvas.height / 2 - 40);
    ctx.fillStyle = 'rgba(255, 255, 255, 0.7)';
    ctx.font = `${Math.min(canvas.width, canvas.height) * 0.035}px Inter, system-ui, sans-serif`;
    ctx.fillText(`Score: ${score}`, canvas.width / 2, canvas.height / 2 + 5);
    const mins = Math.floor(gameTime / 60);
    const secs = Math.floor(gameTime % 60);
    ctx.fillStyle = 'rgba(255, 255, 255, 0.5)';
    ctx.font = `${Math.min(canvas.width, canvas.height) * 0.025}px Inter, system-ui, sans-serif`;
    ctx.fillText(`Survived: ${mins}:${secs.toString().padStart(2, '0')}`, canvas.width / 2, canvas.height / 2 + 40);
    const best = getTopScore();
    ctx.fillStyle = score >= best && score > 0 ? '#FFD700' : 'rgba(255,255,255,0.5)';
    ctx.fillText(score >= best && score > 0 ? `🏆 NEW BEST: ${score}` : `Best: ${best}`, canvas.width / 2, canvas.height / 2 + 72);
    ctx.fillStyle = 'rgba(255, 255, 255, 0.4)';
    ctx.font = `${Math.min(canvas.width, canvas.height) * 0.02}px Inter, system-ui, sans-serif`;
    ctx.fillText('Tap to return', canvas.width / 2, canvas.height / 2 + 105);
}
function renderPlay() {
    ctx.fillStyle = '#0a0a1a';
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    // Danger overlay when near shape limit (>80% full)
    const fullness = shapes.length / MAX_SHAPES;
    if (fullness > 0.8) {
        const pulse = 0.05 + Math.sin(gameTime * 4) * 0.03;
    ...
    ctx.fillStyle = 'rgba(255, 255, 255, 0.5)';
    ctx.font = `${Math.min(canvas.width, canvas.height) * 0.018}px Inter, system-ui, sans-serif`;
    ctx.fillText(tierNames[tier], 16, 90);
    // Error meter ring (top-right)
    const ringX = canvas.width - 50;
    const ringY = 50;
    const ringR = 28;
    renderErrorMeter(ctx, errorMeter, ringX, ringY, ringR);
    // Shape count / screen fullness (below error ring)
    ...
    ctx.font = `${Math.min(canvas.width, canvas.height) * 0.016}px Inter, system-ui, sans-serif`;
    ctx.textAlign = 'right';
    ctx.fillText(`${shapes.length}/${MAX_SHAPES}`, canvas.width - 16, 105);
    // Game time (top-left, below tier)
    const mins = Math.floor(gameTime / 60);
    const secs = Math.floor(gameTime % 60);
    ctx.fillStyle = 'rgba(255, 255, 255, 0.3)';
    ctx.textAlign = 'left';
    ctx.fillText(`${mins}:${secs.toString().padStart(2, '0')}`, 16, 112);
    // Combo (center-top, large)
    if (combo > 1) {
        ctx.fillStyle = combo >= 50 ? '#FFD700' : combo >= 25 ? '#FF6B6B' : 'rgba(255,255,255,0.5)';
        ctx.font = `bold ${Math.min(canvas.width, canvas.height) * 0.04}px Inter, system-ui, sans-serif`;
        ctx.textAlign = 'center';
        ctx.fillText(`${combo} COMBO`, canvas.width / 2, 60);
    }
    // ── Activation messages ───────────────────────────────────
    ...
    if (patternState.lastActivated && patternState.activationTimer > 0) {
        ...
        if (pat) {
            ...
            ctx.font = `bold ${Math.min(canvas.width, canvas.height) * 0.05 * pulse}px Inter, system-ui, sans-serif`;
            ctx.textAlign = 'center';
            ctx.fillStyle = '#FFD700';
            ctx.shadowColor = '#FFD700';
            ctx.shadowBlur = 20;
            ctx.fillText(`✦ ${pat.name} ✦`, canvas.width / 2, y);
            ctx.shadowBlur = 0;
            ctx.font = `${Math.min(canvas.width, canvas.height) * 0.022}px Inter, system-ui, sans-serif`;
            ctx.fillStyle = '#fff';
            ctx.fillText(pat.description, canvas.width / 2, y + 30);
            ctx.restore();
        }
    }
    // Tier-up message
    if (tierUpTimer > 0) {
        ...
        ctx.save();
        ctx.globalAlpha = alpha;
        ctx.font = `bold ${Math.min(canvas.width, canvas.height) * 0.055}px Inter, system-ui, sans-serif`;
        ctx.textAlign = 'center';
        ctx.fillStyle = '#4ECDC4';
        ctx.shadowColor = '#4ECDC4';
        ctx.shadowBlur = 15;
        ctx.fillText(tierUpMessage, canvas.width / 2, y);
        ctx.shadowBlur = 0;
        ctx.restore();
    }
    // Collect-all warning
    if (collectAllActive) {
        ...
        ctx.save();
        ctx.font = `bold ${Math.min(canvas.width, canvas.height) * 0.045 * pulse}px Inter, system-ui, sans-serif`;
        ctx.textAlign = 'center';
        ctx.fillStyle = '#A78BFA';
        ctx.shadowColor = '#A78BFA';
        ctx.shadowBlur = 25;
        ctx.fillText('⚡ TAP ANYWHERE! ⚡', canvas.width / 2, canvas.height * 0.55);
        ctx.shadowBlur = 0;
        ctx.restore();
    }
}
// ── State transitions ─────────────────────────────────────────
function startGame() {
    shapes = generateShapeGrid(canvas.width, canvas.height, 3);
    score = 0;
    combo = 0;
    multiplier = 1.0;
    prevComboMilestone = 0;
        ...
        switch (loop.state) {
            ...
            case GameState.Playing:
                ...
                if (currentTier > prevTier) {
                    ...
                    tierUpTimer = 2.5;
                    triggerRainbowBurst(particles, canvas.width / 2, canvas.height / 2);
                }
                if (tierUpTimer > 0)
                    tierUpTimer -= delta;
                // Check shape limit
