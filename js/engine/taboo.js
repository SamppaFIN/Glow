 ...
 */
export function createTabooState() {
    return {
        activeIds: new Set(),
...
export function updateTaboo(state, shapes, delta, gameTime) {
...
function activateTaboo(state, shapes) {
    if (shapes.length === 0)
        return;
...
function deactivateTaboo(state, shapes) {
...
export function checkTabooHit(shape, state) {
...
export function renderTabooWarning(ctx, shapes, tabooState, gameTime) {
    for (const shape of shapes) {
        if (!shape.taboo)
            continue;
...
export function getTabooTimer(state) {
    return state.active ? state.durationTimer : state.nextEventTimer;
}
export function isTabooActive(state) {
    return state.active;
}
//# sourceMappingURL=taboo.js.map