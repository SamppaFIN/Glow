...
const geoTaps = {};
const GEO_NAMES = { flower: 'Flower of Life', seed: 'Seed of Life', vesica: 'Vesica Piscis', spiral: 'Golden Spiral', sri: 'Sri Yantra', metatron: "Metatron's Cube" };
function recordGeoTap(geo) { geoTaps[geo] = (geoTaps[geo] || 0) + 1; }
let shapes = [];
let score = 0;
let combo = 0;
let multiplier = 1.0;
...
function triggerPattern(pattern, shape) {
    const result = activatePatternEffect(pattern.effect, shapes, W(), H(), score, particles);
...
function startGame() {
    ...
    for (const k of Object.keys(geoTaps))
        delete geoTaps[k];
...
const GEO_ICONS = { flower: '🌸', seed: '🌱', vesica: '💧', spiral: '🌀', sri: '🔥', metatron: '🔮' };
...
function handleNameEntryTap(tapX, tapY) {
    const w = W(), h = H();
