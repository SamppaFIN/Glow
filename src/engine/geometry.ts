...
export function drawSriYantra(
  ...
  for (let i = 0; i < 4; i++) {
    ...
    ctx.lineTo(
      -triangleSize / 2 * Math.cos(Math.PI / 3),
      triangleSize / 2 * Math.sin(Math.PI / 3)
    );
    ctx.lineTo(
      triangleSize / 2 * Math.cos(Math.PI / 3),
      triangleSize / 2 * Math.sin(Math.PI / 3)
...
export type GeometrySymbolName = keyof typeof GeometrySymbols;
