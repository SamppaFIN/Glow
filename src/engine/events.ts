...
export type EventType =
  | 'shape_storm'    // 5 new shapes spawn
  | 'golden_moment'  // All shapes 2x points for 5s
  | 'slow_motion'    // Shape pulses slow down for 3s
  | 'taboo_wave'     // 3 random shapes become taboo
  | 'bubble_party'   // Screen full of bubbles
  | 'shape_swap'     // All shapes randomize geometry
  | 'clear_skies'    // Remove all taboo markers
  | 'mystery_box';   // Random from above
...
export const EVENTS: Record<EventType, RandomEvent> = {
...
const ALL_EVENTS: EventType[] = ['shape_storm','golden_moment','slow_motion','taboo_wave','bubble_party','shape_swap','clear_skies'];

export interface EventState {
  activeEvent: RandomEvent | null;
  eventTimer: number;       // Remaining duration of active event
  nextEventTimer: number;   // Time until next random event
  /** Multiplier applied to all shape scores during golden moment */
  scoreMultiplier: number;
  /** Pulse speed multiplier during slow motion */
...
export function updateEvents(state: EventState, delta: number): RandomEvent | null {
  // Tick active event
...
function triggerRandomEvent(state: EventState): RandomEvent {
  ...
  // Apply effects
  if (type === 'golden_moment') state.scoreMultiplier = 2;
  if (type === 'slow_motion') state.pulseMultiplier = 0.4;

  return event;
}

function endEvent(state: EventState): void {
...
export function applyEventToShapes(
  event: RandomEvent | null,
  shapes: Shape[],
  tabooActiveIds: Set<number>
): void {
  if (!event) return;
  ...
  switch (event.type) {
    case 'taboo_wave': {
      ...
      for (const s of picked) { s.taboo = true; tabooActiveIds.add(s.id); }
      // Clear after 8s
      setTimeout(() => { for (const s of picked) { s.taboo = false; tabooActiveIds.delete(s.id); } }, 8000);
      break;
    }
    case 'clear_skies': {
      for (const s of shapes) { s.taboo = false; }
      tabooActiveIds.clear();
      break;
    }
    case 'shape_swap': {
      const geos: GeometrySymbolName[] = ['flower','seed','vesica','spiral','sri','metatron'];
