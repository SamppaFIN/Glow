...
export type ParticleBehavior =
  | 'burst' | 'spiral' | 'confetti' | 'cosmic' | 'magnetic' | 'fizzle'
  | 'smoke' | 'bubble' | 'rainbow' | 'fire' | 'water' | 'lightning';
...
export function triggerSeedBurst(p: Particle[], x: number, y: number): void {
  triggerBurst(p, x, y, 'rgba(245, 158, 11, 0.9)', { count: 36, speed: 5, size: 6, effect: 'cosmic' });
}
export function triggerVesicaBurst(p: Particle[], x: number, y: number): void {
  triggerBurst(p, x, y, 'rgba(6, 182, 212, 0.8)', { count: 24, speed: 2, size: 5, effect: 'water' });
}
export function triggerMetatronBurst(p: Particle[], x: number, y: number): void {
  triggerBurst(p, x, y, 'rgba(138, 92, 246, 0.9)', { count: 20, speed: 5, size: 4, effect: 'lightning' });
}
export function triggerFlowerBurst(p: Particle[], x: number, y: number): void {
  triggerBurst(p, x, y, 'rgba(255, 255, 255, 0.6)', { count: 15, speed: 1, size: 6, effect: 'smoke' });
}
export function triggerSriBurst(p: Particle[], x: number, y: number): void {
  triggerBurst(p, x, y, 'rgba(255, 107, 107, 0.9)', { count: 20, spread: Math.PI * 0.6, speed: 3, size: 4, effect: 'fire' });
}
export function triggerSpiralBurst(p: Particle[], x: number, y: number): void {
...
export function triggerGeometryBurst(p: Particle[], x: number, y: number, geometry: string): void {
...
export function triggerBubbleBonus(p: Particle[], x: number, y: number): void {
...
export function triggerRainbowBurst(p: Particle[], x: number, y: number): void {
  triggerBurst(p, x, y, '#FFD700', { count: 40, speed: 4.5, size: 5, effect: 'rainbow' });
}

export function triggerMaxComboBurst(p: Particle[], x: number, y: number): void {
...
export function triggerComboBurst(p: Particle[], x: number, y: number, level: number): void {
  if (level >= 50) triggerMaxComboBurst(p, x, y);
  else if (level >= 25) triggerRainbowBurst(p, x, y);
  else triggerBurst(p, x, y, '#FFD700', { count: 20 + level, speed: 4, effect: 'cosmic' });
}

export function triggerFizzle(p: Particle[], x: number, y: number): void {
...
export function spawnRain(p: Particle[], w: number, count = 3): void {
...
export function spawnSnow(p: Particle[], w: number, count = 2): void {
  for (let i = 0; i < count; i++) {
...
export function spawnRewardBubbles(p: Particle[], w: number, h: number, count = 4): void {
